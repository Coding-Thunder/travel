...
const BookHotelPage: React.FC = () => {
    ...
    // Redux: selected hotel & search criteria
    const selectedOffer = useSelector((state: RootState) => state.hotel.selectedOffer);
    //   const searchCriteria = useSelector((state: RootState) => state.hotel.);
    ...
    const handleFinalSubmission = async (e: React.FormEvent) => {
        ...
        if (!selectedOffer) {
            toast({
                title: "Error",
                description: "No hotel selected for booking.",
                variant: "destructive",
            });
            return;
        }
        ...
        try {
            await apiService.createHotelBooking(bookingDetails); // Call /bookings/hotel
            setLoading(false);
            toast({
                title: "Success",
